...
if __name__ == '__main__':
    args =  load_model()
    source = "./../scienceDaily/text/"
    for i,files in enumerate(os.listdir(source)):
        print(files)
        types= "train" if i%10!=0 else "validation"
        f = open(source+files,"r").read()
        g = open(source[:-5]+"abstract/"+files[:-4]+"_abstract.txt","r").read()

        data_builder.str_format_to_bert_train(f, g, args,'./../scienceDaily/pt/sd.'+types+str(i)+'.pt') 
